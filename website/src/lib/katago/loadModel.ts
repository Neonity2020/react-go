// KataGo model loader — parses .bin.gz model files
// Ported from web-katrain (https://github.com/Sir-Teo/web-katrain)
...
export function decompressGzip(data: ArrayBuffer): Uint8Array {
  const raw = new Uint8Array(data);
  // Check if data is actually gzip (starts with 0x1f 0x8b)
  if (raw[0] === 0x1f && raw[1] === 0x8b) {
    return inflate(raw);
  }
  // Already decompressed (browser auto-decompressed via Content-Encoding)
  return raw;
}
...
export function parseKataGoModelV8(data: Uint8Array): ParsedKataGoModelV8 {
  ...
  if (modelVersion >= 15) {
    for (let i = 0; i < 7; i++) p.readInt(); // unused model-level params
  }
  ...
  if (modelVersion >= 15) {
    for (let i = 0; i < 6; i++) p.readInt(); // unused trunk params
  }
